...
RUN mkdir -p /usr/local/src/xblock-react-5

# Set the created directory as a Docker volume
VOLUME ["/usr/local/src/xblock-react-5"]
...
RUN echo "pip install -r /usr/local/src/xblock-react-5/requirements.txt" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "pip install -e /usr/local/src/xblock-react-5" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "cd /usr/local/src/xblock-react-5 && make compile_translations && cd /usr/local/src/xblock-sdk" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
